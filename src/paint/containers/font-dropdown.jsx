...
import {getImportedFonts} from '../lib/imported-fonts';
...
class FontDropdown extends React.Component {
    ...
    getFontName (font) {
        ...
        case Fonts.JAPANESE:
            return '日本語';
        default:
            // 自定义字体以带引号的 CSS family 形式存储，展示时去掉引号
    ...
    handleHoverCustom (font) {
        if (this.dropDown && this.dropDown.isOpen()) {
            this.props.changeFont(font);
        }
    }
    handleFontImported (family) {
        // 组件已完成 FontFace 注册；同步下拉列表并直接应用该字体
        this.setState({customFonts: getImportedFonts()});
    ...
    render () {
        return (
            <FontDropdownComponent
                ...
                onFontImported={this.handleFontImported}
